...
async function headless(url, date) {
 ...
 await pPage.setUserAgent('Mozilla/5.0 (Macintosh; Intel Mac OS X 10_13_2) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3987.132 Safari/537.36');
 await pPage.setViewport({width: 1280, height: 800, isMobile: false})
 const response = await pPage.goto(url, 
  {
      timeout: 25000, 
  ...
  if (response._status < 400) {
      await pPage.waitFor(3000);
      let html = await pPage.content();
      let page = await cheerio.load(html)
      return {browser, page};  
  }
}
