...
let isSSLLoaded = false;

/**
 * Some sources require non-standard SSL certificates. This is the place where we load them.
 */
export const loadSSLCerts = async () => {
  if (isSSLLoaded === false) {
 ...
 */
export const fetch = async (url, type, date = process.env.SCRAPE_DATE || datetime.getYYYYMD(), alwaysRun = false) => {
 ...
 * @param {*} alwaysRun fetches from URL even if resource is in cache, defaults to false
 */
export const page = async (url, date, alwaysRun = false) => {
  const body = await fetch(url, 'html', date, alwaysRun);
 ...
 * @param {*} alwaysRun fetches from URL even if resource is in cache, defaults to false
 */
export const json = async (url, date, alwaysRun = false) => {
  const body = await fetch(url, 'json', date, alwaysRun);
 ...
 * @param {*} alwaysRun fetches from URL even if resource is in cache, defaults to false
 */
export const csv = async (url, date, alwaysRun = false) => {
  return new Promise(async (resolve, reject) => {
    const body = await fetch(url, 'csv', date, alwaysRun);
 ...
 * @param {*} alwaysRun fetches from URL even if resource is in cache, defaults to false
 */
export const headless = async (url, date = process.env.SCRAPE_DATE || datetime.getYYYYMD(), alwaysRun = false) => {
