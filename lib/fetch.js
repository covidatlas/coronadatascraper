...
async function fetchWayback(url, type, date) {
  console.log('  🕸️  Attempting to fetch %s from the wayback machine', url);

  let formattedDate = datetime.getYYYYMMDD(new Date(date));
  let results = await json(`http://archive.org/wayback/available?url=${url}&timestamp=${formattedDate}`, false);

  if (results.timestamp === formattedDate && results.archived_snapshots && results.archived_snapshots.closest) {
    let waybackURL = results.archived_snapshots.closest.url;
    waybackURL = waybackURL.replace(/(\/web\/\d+)/, '$1_id');
    let body = await fetch(waybackURL, type, false);
...
async function getCachedFile(url, type, date) {
  ...
  if (await fs.exists(filePath)) {
    ...
    if (date === '2020-3-12') {
      return await fetchWayback(url, type, date);
    }
    return undefined;
  }
