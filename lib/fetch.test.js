...
describe('Fetching', () => {
  ...
  describe('fetch', () => {
    test('when fetching an example url, we get the correct data back', async () => {
      ...
      const body = await fetch.fetch(`https://postman-echo.com/get?${key}=${value}`, 'json', false);
    ...
    test('when fetching a URL in cache with alwaysRun=true, we fetch a new version', async () => {
      ...
      const body = await fetch.fetch(url, type, date, true);
  ...
  describe('headless', () => {
    test('when fetching an example url, we get the correct data back', async () => {
      ...
      mockFs.restore();
      const body = (await fetch.headless(`https://postman-echo.com/get?${key}=${value}`, 'json', false)).text();
